Request.sendFeedbackRequest = (info, next) => {
		...
		Request.app.models.client.findById(info.client.id, 
(err, clientInstance) => {
			if (err) {
   				return next(err);
			}
...
<label for="rating-${info.products[i].id}-1" style="font-size: 1.3em; color: #eed034;">★</label>
      ...
      <label for="rating-${info.products[i].id}-2" style="font-size: 1.3em; color: #eed034;">★★</label>
						  ...
						  <td>
							...
							<label for="rating-${info.products[i].id}-3" style="font-size: 1.3em; color: #eed034;">★★★</label></td>
						<td>
							...
							<input type="radio" id="rating-${info.products[i].id}-5" name="feedbacks[${info.products[i].id}][totalratingscore]" value="5"><label for="rating-${info.products[i].id}-5" style="font-size: 1.3em; color: #eed034;">★★★★★</label>
						</td>`;
						formProductsFields += `
					<tr>
					...
					<tr style="text-align: left">
						<td colspan="5">
							...
							</textarea>              <input type="hidden" name="feedbacks[${info.products[i].id}][productnumber]" value="${info.products[i].productnumber}">
						</td>
					</tr>
					<tr><td colspan="5"><hr style="display: block; height: 1px; border: 0; border-top: 1px solid #fff; margin: 1em; padding: 0;"></td>
					</tr>`;
